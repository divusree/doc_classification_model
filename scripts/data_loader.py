...
class DataLoader:
    ...
    def load_data_from_url(self, url):
        ...
        try:
            response = requests.get(
                url, stream=True, headers=self.headers, allow_redirects=True, timeout=5
            )
    ...
    def create_dataset_json(self, df, save_path=os.path.join("scripts", "datasets", "dataset.json")):
        """
...
def main():
    ...
    parser.add_argument(
        "--dataset_path", type=str, required=True, help="Path to the dataset Excel file"
    )
    parser.add_argument(
        "--train_save_path",
        type=str,
        default=os.path.join("scripts", "datasets", "train_json.json"),
        help="Path to save the training JSON file",
    )
    parser.add_argument(
        "--test_save_path",
        type=str,
        default=os.path.join("scripts", "datasets", "test_json.json"),
        help="Path to save the testing JSON file",
    )
    args = parser.parse_args()

    df = pd.read_excel(args.dataset_path, sheet_name=None)
    if not os.path.exists(os.path.join("scripts", "datasets")):
        os.makedirs(os.path.join("scripts", "datasets"))
